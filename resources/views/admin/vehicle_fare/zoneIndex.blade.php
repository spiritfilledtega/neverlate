...
<div class="box">
    ...
    <div id="js-vehicle_fare-partial-target">
        <include-fragment src="fetch/{{$zone->id}}">
            ...
            $(function() {
                $('body').on('click', '.pagination a', function(e) {
                    ...
                    $.get(url, $('#search').serialize(), function(data) {
                        $('#js-vehicle_fare-partial-target').html(data);
                    });
                });
                ...
                $('#search').on('click', function(e) {
                    ...
                    fetch('fetch/'+"{{$zone->id}}"+'?search=' + search_keyword)
                        .then(response => response.text())
                        .then(html => {
                            document.querySelector('#js-vehicle_fare-partial-target').innerHTML = html
                        });
                });

                $('.filter,.resetfilter').on('click', function() {
                    let filterColumn = ['active', 'approve', 'available','zone'];
                    ...
                    $.each(filterColumn, function(index, value) {
                        if (className.hasClass('resetfilter')) {
                            ...
                            if(value == 'zone') $('#service_location_id').val('all')
                            query = '';
                        } else {
                            if ($('input[name="' + value + '"]:checked').attr('id') != undefined) {
                                ...
                                var zone = $('#service_location_id').val()
                    ...
                    if (isConfirm) {
                        ...
                        $.ajax({
                            ...
                            success: function(res) {
                                ...
                                fetch('fetch/'+"{{$zone->id}}"+'?search=' + search_keyword + '&' + query)
                                    .then(response => response.text())
                                    .then(html => {
                                        document.querySelector('#js-vehicle_fare-partial-target')
                                            .innerHTML = html
                                    });
